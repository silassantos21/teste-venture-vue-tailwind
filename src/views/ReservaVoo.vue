...
const reservar = () => {
  alert(
    isReturn.value
      ? `Reserva de voo com ida marcada para a data ${dataIda.value} e retorno na data ${dataVolta.value} feita com sucesso.`
      : `Reserva de voo com ida marcada para a data ${dataIda.value} feita com sucesso.`
  )
}
        ...
        <div class="w-3/4">
          <BaseCard>
            ...
            <div class="grid grid-cols-1 items-center" style="text-align: center; place-items: center;">
              <div class="w-1/4">
                <select required data-type="selectTipoViagem" v-model="vooOpcao" class="mb-6 text-black-70 bg-white px-3 py-2 transition-all cursor-pointer hover:border-blue-600/60 border rounded-lg outline-blue-600/80 appearance-none invalid:text-black-30 w-64">
              ...
              <div class="w-1/4">
                <input data-type="dataIda" v-model="dataIda" datepicker-autohide type="text" onfocus="(this.type='date')" onblur="(this.type='text')" class="mb-6 bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Selecione Ida">
              </div>
              <div class="w-1/4">
                <input data-type="dataVolta" :disabled="!isReturn" v-model="dataVolta" datepicker-autohide type="text" onfocus="(this.type='date')" onblur="(this.type='text')" :class="border-gray-300" class="mb-6 bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg disabled:text-gray-400 focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5"  placeholder="Selecione Volta">
              </div>
              <div class="w-1/4" v-show="dataIda">
                <BaseBtn data-type="botaoReservar" class="mb-3 mr-3 " :disabled="!canBook" :class="!canBook ? 'border-gray-300 bg-gray-300 text-gray-500' : 'border border-info text-info hover:bg-info hover:text-white '" xl @click="reservar">
                  Reservar
                </BaseBtn>
              </div>
